<template>
    <div id="order">
        <div class="container">
            <!--导航切换-->
            <div class="topnav">
                <p @click="changeStatus(1)" :class="ordertype==1?'blue':''">全部工单({{allnum}})</p>
                <p @click="changeStatus(2)" :class="ordertype==2?'blue':'' ">待处理工单 <span v-if="waitnum">({{waitnum}})</span></p>
            ...
            <div class="table">
                <div class="container">
                    ...
                    <div class="handle-box" v-if="ordertype!=2">
                        <el-select v-model="searchForm.status" placeholder="工单状态" class="handle-select mr10"
                                   v-if="ordertype==1">
                    ...
                    >
                        ...
                        <el-table-column prop="type" label="类型" min-width="60"     align="center">
                            <template slot-scope="scope">
                                <span v-if="scope.row.type==1?true:''">SQL</span>
                                <span v-if="scope.row.type==2?true:''">Apollo</span>
                                <span v-if="scope.row.type==3?true:''">Nginx</span>
                                <span v-if="scope.row.type==4?true:''">其他</span>
                            </template>
                        </el-table-column>
                        <el-table-column prop="creator" label="创建人" min-width="70"     align="center">
                        </el-table-column>
                        <el-table-column prop="status" label="状态" min-width="70"  >
                            <template slot-scope="scope">
                                ...
                                <span v-if="scope.row.status==4?'true':''"><span class="smallRound smallRound3"></span>已完成</span>
                        ...
                        <el-table-column prop="currentProcessor" label="当前处理人" min-width="90"     align="center">
                        </el-table-column>
                        <el-table-column prop="title" label="标题" :show-overflow-tooltip="true" min-width="220"  >
                        </el-table-column>
                        <el-table-column
                                prop="createTime"
                                label="提交时间"
                        ...
                        <el-table-column label="操作" align="center" min-width="240" v-if="ordertype==1">
                            <template slot-scope="scope">
                                ...
                                <el-button type="text" icon="el-icon-delete" class="red" @click="revoke(1, scope.row)"
                                           v-if="scope.row.status==1&&userid==scope.row.creatorId">删除
                                </el-button>
                            </template>
                        </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            search() {
                ...
                this.$axios({
                    url: "/wo/queryList",//请求路径
                    type: "get",//请求方式
                    params: {
                        status: this.searchForm.status,
                        submitUser: this.searchForm.submitUser,
            ...
            //撤销或者删除，弹出模态框，type:0，撤销，1，删除， 2，提交
            revoke(type, row) {
                this.deletype = type;
